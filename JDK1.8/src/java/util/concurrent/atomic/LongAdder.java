...
public class LongAdder extends Striped64 implements Serializable {
    ...
    public void add(long x) {
        ...
        if ((as = cells) != null || !casBase(b = base, b + x)) {
            ...
            // 如果cells[threadLocalRandomProbe%cells.length]的位置为null，这说明这个位置从来没有线程做过累加，需要进入if继续执行，在这个位置创建一个新的Cell对象；

            //4. !(uncontended = a.cas(v = a.value, v + x))：尝试对cells[threadLocalRandomProbe%cells.length]位置的Cell对象中的value值做累加操作,
