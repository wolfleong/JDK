...
public abstract class AbstractQueuedSynchronizer
    ...
    static final class Node {
         ...
         *
         *  CANCELLED: 1, SIGNAL: -1, CONDITION: -2, PROPAGATE: -3, 默认: 0
         *
         */
    ...
    private Node enq(final Node node) {
        for (;;) {
            //获取尾节点
            Node t = tail;
            //如果尾节点为 null, 则表示 head 和 tail 都没初始化
            if (t == null) { // Must initialize
                //创建空的 Node, 并 cas 初始化 Head , 如果成功则初始化 tail
                if (compareAndSetHead(new Node()))
                    tail = head;
            } else {
                //和上一个方法 addWaiter 是一样的
                //先设置 node 的前驱节点引用
                node.prev = t;
                //再 cas 尝试替换 tail 节点到
    ...
    private void cancelAcquire(Node node) {
        ...
        if (node == tail && compareAndSetTail(node, pred)) {
            ...
            if (pred != head &&
                ...
                pred.thread != null) {
                Node next = node.next;
                //这里只处理了 pred 的 next 的引用, 并没有处理 next.pred 引用
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        // 所以下面这块代码说的是将当前节点的prev指向 waitStatus<=0 的节点，
        ...
        if (ws > 0) {
             ...
             */
            do {
                //往前遍历, 直到找到 waitStatus <= 0 的
                //注意: 这里有设置 pred 引用, 那些被忽略掉的中间节点是没有清除引用的, 画一下图就明白
                node.prev = pred = pred.prev;
            } while (pred.waitStatus > 0);
            //重新设置 next 引用
            pred.next = node;
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (;;) {
                ...
                // p == head 说明当前节点虽然进到了阻塞队列，但是是阻塞队列的第一个，因为它的前驱是 head
    ...
    public final void acquire(int arg) {
        //先用 tryAcquire(arg) 尝试获取锁, 成功则返回
        //尝试获取锁不成功,  addWaiter 则创建当前线程的节点, 并添加到阻塞队列尾部
        // acquireQueued 处理线程挂起与唤醒
        if (!tryAcquire(arg) &&
            acquireQueued(addWaiter(Node.EXCLUSIVE), arg))
            //重新设置线程中断状态, 因为里面的中断状态被清除了
            selfInterrupt();
    }
    ...
    public final boolean hasQueuedPredecessors() {
        ...
        //s 头节点
        Node s;
        //如果 head != tail 表示队列起码有两个不同的节点
        //如果 h.next 为 null , 则只有一种情况, 在并发的情况下,
        // addWaiter() 刚 cas 替换 tail 节点成功, 但还没来得及设置 next , 所以 h.next == null 也算是有节点排队
        // h.next 不为 null , 则判断一下节点的线程是否为当前线程, 如果是则当前线程为头节点, 那么返回 false 表示可以尝试抢锁
        return h != t &&
    ...
    final boolean isOnSyncQueue(Node node) {
        // 移动过去的时候，node 的 waitStatus 会置为 0， signal 方法中设置
    ...
    final boolean transferForSignal(Node node) {
        ...
        // 如果 ws <= 0, 那么 compareAndSetWaitStatus 将会被调用，因为节点入队后，需要把前驱节点的状态设为 Node.SIGNAL(-1)
        // 因为默认最后一个节点肯定是非 SIGNAL(-1) 的
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        /**
         * 唤醒等待了最久的线程, 其实就是，将这个线程对应的 node 从条件队列转移到阻塞队列
        ...
        /**
         * 此方法用于判断是否在线程挂起期间发生了中断，如果发生了中断，是 signal 调用之前中断的，还是 signal 之后发生的中断
         *
        ...
        private int checkInterruptWhileWaiting(Node node) {
            // 1. 如果在 signal 之前已经中断, 也就是还在 await 中被中断，返回 THROW_IE
        ...
        public final void await() throws InterruptedException {
            ...
            //注意，前面我们说过，不管有没有发生中断，都会进入到阻塞队列，而 acquireQueued(node, savedState) 的返回值就是代表线程是否被中断。
            // 如果返回 true，说明被中断了，而且 interruptMode != THROW_IE，说明在 signal 之前就发生中断了，
            ...
            //也就是这个 node 节点移到阻塞队列但没断开连接, 也就相当于取消的节点了, 那么, 便要将等待链表中的节点清除
